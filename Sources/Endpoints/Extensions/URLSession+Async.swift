...
extension URLSession {
    public func response<T: Endpoint>(in environment: EnvironmentType, with endpoint: T) async throws where T.Response == Void {
    ...
    public func response<T: Endpoint>(in environment: EnvironmentType, with endpoint: T) async throws -> T.Response where T.Response == Data {
        let urlRequest = try createUrlRequest(in: environment, for: endpoint)
    ...
    public func response<T: Endpoint>(in environment: EnvironmentType, with endpoint: T) async throws -> T.Response where T.Response: Decodable {
        let urlRequest = try createUrlRequest(in: environment, for: endpoint)
